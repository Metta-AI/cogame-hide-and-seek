...
##      deliberately NOT src/hns/replay_runtime.initReplayRuntime, which
...
proc renderBoardFrame*(sim: var SimServer, scale = RenderScale): Image =
  ...
  compositeBoard(
    sim.buildSpriteProtocolUpdates(
      state, next, @[], sim.tickCount, false, 1, sim.tickCount, false, false,
      -1
    ).parseSpritePacket(), scale
  )
