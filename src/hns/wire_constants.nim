...
## static wasm bundle. Clients read `window.HNS_WIRE` and keep their old
## literals only as fallbacks for raw file:// opens of the un-spliced sources.
...
const WireConstantsJs* =
  ...
  "window.HNS_WIRE={speeds:[0.5," & jsIntArray(PlaybackSpeeds)[1..^1] &
  ",fps:" & $TargetFps &
  ",chromeSpriteId:" & $BroadcastChromeSpriteId &
  ",turnTicks:" & $DefaultTurnTicks &
  ",shoutTicks:" & $ShoutTicks &
  ",beats:[\"release\",\"spotted\",\"lock\",\"vault\",\"sealed\",\"fallback\",\"gameover\"]" &
  "};"

const WireConstantsMarker* = "<!-- WIRE_CONSTANTS -->"
  ## The placeholder both client HTML files carry where the block belongs
  ## (before any script that reads window.HNS_WIRE).
