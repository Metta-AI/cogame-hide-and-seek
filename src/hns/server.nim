...
const
  ...
  # The two role kits (nano-banana renders of the Softmax cog), served as
  # static PNGs so a page can show the kit a policy is wearing. The board
  # sprites are composited from the same masters by rig_art.
  CogArtAssets = [
    ("/client/cog_hider.png", staticRead("../../data/cog_hider.png")),
    ("/client/cog_seeker.png", staticRead("../../data/cog_seeker.png")),
  ]
...
proc liveProgressMaxTick(config: GameConfig): int =
  ## Returns the live viewer tick-bar budget: the whole EPISODE, both games.
  config.maxTicks * max(1, config.maxGames)
...
proc httpHandler(request: Request) =
  if request.path == HealthPath and request.httpMethod == "GET":
    ...
    request.respond(200, headers, "HNS server")
...
proc buildRewardPacket(sim: SimServer): string {.measure.} =
  ...
  for player in sim.players:
    ...
    if accountIndex >= 0:
      result.addStatLine("seen", identity, player.seatSeenTicks)
      result.addStatLine("sealed", identity, player.sealedTicks)
      result.addStatLine("locks", identity, player.locks)
      result.addStatLine("grabs", identity, player.grabs)
      result.addStatLine("vaults", identity, player.vaults)
...
proc runServerLoop*(
  ...
  # --- commanded mode -------------------------------------------------------
  # `num_agents` seats drive exactly `num_agents` cogs, one each. The seats
  # join exactly as the starter's players do (slots 0..num_agents-1,
  # token-checked); from then on every actuator mask comes from the control
  # layer rather than from a socket, and the seats send no inputs at all.
  let squadMode = not replayLoaded and config.numAgents > 0
  var
    engine =
    ...
    turnViews: seq[JsonNode] = @[]
    turnSources: seq[DirectiveSource] = @[]
    turnLatencies: seq[int] = @[]
  ...
  while true:
    ...
    # iteration: `wallClockBudgetSeconds` (660 s) is 55% of the assumed
    # 1200 s episodeTimeoutSeconds, so the episode always settles and scores
    # itself rather than being silently discarded for overrunning.
    if squadMode and not deadlineHit and
      ...
      echo "wall-clock budget of ", config.wallClockBudgetSeconds,
        "s reached; settling the episode from the exposure counts at this tick"
      # THE LOAD-BEARING STOP: written as one record and applied by the SAME
      # proc on record and on playback, so the record -> re-derive check
      # passes for `wall_clock` exactly as it does for `full_time`.
      replayWriter.writeChat(tickTime(sim.tickCount), 0,
        stopRecord(sim.tickCount, EndRuleWallClock))
      sim.forceWallClockStop()
      # A record is applied at the START of the step that leaves its tick, so
      # the recording has to leave ONE MORE TICK for playback to apply it in.
      # Stepping once with zero inputs and hashing that tick is what makes the
      # record -> re-derive check pass for a stopped episode as well as for
      # full time (the particle-worlds 13c66d7 scar).
      block:
        let quiet = newSeq[InputState](sim.players.len)
        for cogIndex in 0 ..< sim.players.len:
          replayWriter.writeInputMaskChange(
            tickTime(sim.tickCount), cogIndex, 0)
        sim.step(quiet, quiet)
    ...
    {.gcsafe.}:
      withLock appState.lock:
        ...
        if not replayLoaded and not squadMode and
        ...
        if not replayLoaded:
          ...
          # --- roster completion -------------------------------------------
          # One cog per seat, so there is nothing to fill in: the roster is
          # complete the moment the six seats are seated. If the lobby budget
          # expired and a no-show has been reported, the remaining slots are
          # taken by TRUSTED joins carrying only the cogs' ANONYMOUS aliases —
          # the replay's join stream leaks no policy identity — so a missing
          # seat cannot stop the clock (§End conditions).
          if squadMode and not squadsBuilt and sim.phase == Lobby and
              (sim.players.len >= config.numAgents or squadForceStart):
            for order in sim.players.len ..< config.numAgents:
              try:
                discard sim.addPlayer(
                  sim.seatAlias(order), order, "", trusted = true)
              except HnsError as error:
                echo "roster completion failed at seat ", order, ": ",
                  error.msg
                break
              replayWriter.writeJoin(
                tickTime(sim.tickCount), order, sim.seatAlias(order),
                order, "")
              while replayWriter.lastMasks.len < sim.players.len:
                replayWriter.lastMasks.add(0)
              while liveOverlays.len < sim.players.len:
                liveOverlays.add(DebugOverlay())
              if order < sim.deadSeats.len:
                sim.deadSeats[order] = true
            squadsBuilt = sim.players.len >= config.numAgents
            if squadsBuilt:
              ...
              echo "roster complete: ", sim.players.len, " cogs, ",
                config.numAgents, " seats, room ", sim.roomName
    ...
    # ------------------------------------------------------------------
    #  THE DECISION TURN, then the control-compiled actuator masks. This is
    #  the determinism boundary — the control layer and the LLM live on THIS
    #  side of it, and only the masks below are recorded, so the wasm viewer
    #  re-derives the whole match from them without ever running either.
    # ------------------------------------------------------------------
    if squadMode and squadsBuilt and sim.phase == Playing:
      let
        ...
        turnIndex = sim.gameTick() div turnTicks
        turnKey = sim.gameIndex * 1_000_000 + turnIndex
      engine.ctl.observeEnemies(sim)
      if sim.gameTick() mod turnTicks == 0 and turnKey != lastTurnKey:
        lastTurnKey = turnKey
        sim.turnIndex = turnIndex
        sim.emitEvent(TurnStart, amount = turnIndex,
          subject = phaseText(sim.matchPhase))
        let records = engine.turn(
          sim, turnIndex, elapsedSeconds, turnViews, turnSources,
          turnLatencies)
        for record in records:
          replayWriter.writeChat(tickTime(sim.tickCount), 0, record)
        for seat in 0 ..< engine.orders.len:
          if not engine.haveOrder[seat]:
            continue
          ...
          if engine.lastResult[seat] == "unknown_object":
            inc sim.ordersRejected[min(seat, sim.ordersRejected.high)]
          let
            directive = Directive(
              slot: seat,
              order: engine.orders[seat],
              source: turnSources[seat],
              latencyMs: turnLatencies[seat]
            )
            record = directive.boundedOrderRecord(
              sim.gameIndex + 1, turnIndex, sim.cogAlias(index),
              engine.lastResult[seat], viewForRecord(turnViews[seat]))
          replayWriter.writeChat(tickTime(sim.tickCount), seat, record)
          sim.pushFeedDirective(record)
          sim.emitEvent(
            Directive, source = index, subject = $turnSources[seat],
            amount = turnIndex, content = engine.orders[seat].radio)
          ...
          if engine.orders[seat].say.len > 0:
            if sim.applyShout(index, engine.orders[seat].say):
              replayWriter.writeChat(
                tickTime(sim.tickCount), index, engine.orders[seat].say)
      # Compile one mask per COG, in slot order, every tick.
      inputs = newSeq[InputState](sim.players.len)
      for cogIndex in 0 ..< sim.players.len:
        let seat = sim.players[cogIndex].joinOrder
        var order: Order
        if seat < engine.orders.len and engine.haveOrder[seat]:
          order = engine.orders[seat]
        else:
          # No order yet: the published `burrow` baseline, which is what
          # "no failure mode leaves a cog unactuated" means here.
          order = engine.burrowFor(sim, seat, 0)
        var mask = engine.ctl.compileMask(sim, order, cogIndex)
        if sim.frozenSeeker(cogIndex):
          mask = 0    ## a frozen seeker's mask is forced to zero.
      ...
      # NOT playing (the game-over hold between the two games of an episode):
      # the server steps with all-zero inputs, so the replay has to be told
      # that. Without it, playback keeps re-applying the last masks of the
      # previous game and the first Playing tick of the next one sees a
      # different `prev` — which decides whether a fresh press fires, and
      # diverges the hash chain at exactly that tick.
      for cogIndex in 0 ..< sim.players.len:
    ...
    if replayLoaded:
      ...
      for _ in 0 ..< playbackSpeed(liveSpeedIndex):
        ...
        try:
          ...
          if not squadMode:
            raise
          echo "hide-and-seek: SIM GUARD tripped at tick ", sim.tickCount,
            ": ", guard.msg
          faultRule = EndRuleSimFault
          faultDetail = guard.msg
        except CatchableError as error:
          if not squadMode:
            raise
          echo "hide-and-seek: HOST ERROR at tick ", sim.tickCount, ": ",
            error.msg
          faultRule = EndRuleHostError
          faultDetail = error.msg
        if faultRule.len > 0:
          # The fault stop is LOAD-BEARING too: recorded as one record and
          # applied by the same proc on record and on playback.
          replayWriter.writeChat(tickTime(sim.tickCount), 0,
            $(%*{"k": "stop", "tick": sim.tickCount, "endRule": faultRule,
                 "detail": sanitizeLine(faultDetail, MaxFallbackDetailRunes)}))
          sim.forceFaultStop(faultDetail)
        ...
        if phaseBeforeStep != GameOver and sim.phase == GameOver:
          gamesPlayed = sim.gameMargins.len
          lastTurnKey = -1
          echo "game ", gamesPlayed, " done; margin ",
            (if sim.gameMargins.len > 0: sim.gameMargins[^1] else: 0),
            " permille (hiders)"
        if gamesPlayed >= config.maxGames and sim.gameOverTimer <= 0:
          quitAfterFrame = true
          break
    ...
    if profileShouldDump(sim.gameTick()):
      finishProfileTrace()
    ...
    if quitAfterFrame:
      if squadMode:
        ...
        # table), so the replay is SELF-SUFFICIENT — the outcome would
        # otherwise live only at COGAME_RESULTS_URI, which a spectator holding
        # the bytes cannot read. Never applied as a shout at playback (a
        # leading '{' marks a control record), so the hash chain is untouched.
        sim.settleEpisode()
      ...
      if runtimeConfig.resultsUri.len > 0:
        let scoresJson = sim.roomResultsJson() & "\n"
        runtimeConfig.writeResults(scoresJson)
      elif saveScoresPath.len > 0:
        writeFile(saveScoresPath, sim.roomResultsJson() & "\n")
