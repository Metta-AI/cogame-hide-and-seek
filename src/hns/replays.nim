...
type
  ...
  ReplayPlayer* = object
    ...
    exposureRibbon*: seq[int]
      ## The per-tick exposure bit of the whole episode, run-length encoded
      ## as [firstTick, len, exposed, len, hidden, ...] from the load-time
      ## pre-scan. It is what lets the exposure ribbon draw at FULL WIDTH on
  ...
  ReplayScan* = ref object
    ...
    interval: int
    maxTick: int
    ribbonState: int           ## -1 = nothing recorded yet.
    ribbonRun: int
...
const
  ...
  HnsReplayMagic = "COWLDHNS"
  HnsReplayFormatVersion = 1'u16
  HnsReplaySpec = ReplaySpec(
    magic: HnsReplayMagic,
    formatVersion: HnsReplayFormatVersion,
    gameName: GameName,
...
type ReplayStaticBakes = object
  ...
  ## immutable map).
  mapPixels, mapRgba: seq[uint8]
  walkMask, wallMask: seq[bool]
    ## `objectMask` and `fovBlocked` are deliberately NOT here: the furniture
    ## moves, so both are live state a keyframe must carry.
...
proc deserializeReplaySim*(bytes: string, donor: var SimServer): SimServer =
  ...
  ## Only the ROOM bake is donated. `objectMask` and `fovBlocked` ride the
  ## keyframe itself, because the furniture is dynamic geometry: donating a
  ## later tick's furniture into an earlier keyframe is exactly how a seek
  ## would render a fort that has not been built yet.
...
proc applyReplayEvents(replay: var ReplayPlayer, sim: var SimServer) =
  ...
  while replay.chatIndex < replay.data.chats.len and
    ...
    # CONTROL records (register / directive / fallback / budget_guard / stop
    # / result) ride the chat stream as JSON objects and are NOT shouts: the
    # live server never applied them as shouts either, so applying them here
    # would move the hash chain. `stop` is the one exception that touches the
    # sim, and it is applied through the SAME proc the recording used, which
    # is what makes the record -> re-derive check pass for `wall_clock` and
    # `sim_fault` as well as `full_time`. Everything else is a cog's real
    # in-game shout, hashed state both sides hear.
...
proc scanTeamLead(sim: SimServer): seq[int] =
  ## The momentum metric: the CUMULATIVE (hidden - seen) tick margin across
  ## the whole episode, from the HIDING trio's point of view, plus the sealed
  ## count so the graph can shade the sealed spans. A lives series would be
  ## meaningless here — nobody can be killed.
  var hidden = sim.hiddenTicks
  var seen = sim.seenTicks
  for i in 0 ..< sim.gameHidden.len:
    hidden += sim.gameHidden[i]
    seen += sim.gameSeen[i]
  @[hidden - seen, sim.sealedCount(), (if sim.exposedNow: 1 else: 0)]
...
proc initReplayScan*(
  ...
  replay.leadSeries = @[]
  replay.lullSpans = @[]
  replay.exposureRibbon = @[]
  replay.beatEvents = newJArray()
  replay.scanDone = false
  var scan = ReplayScan(interval: max(interval, 1), ribbonState: -1)
  scan.sim = initialSim
...
proc advanceReplayScan*(replay: var ReplayPlayer, maxTicks: int) =
  ...
  while stepsLeft > 0 and scan.builder.playing and
    ...
    # The per-tick exposure bit of the hunt phase, run-length encoded: this
    # is the ribbon, and pre-scanning it is what lets it draw at FULL WIDTH
    # on the first frame instead of growing in.
    if scan.sim.phase == Playing and scan.sim.matchPhase == phaseHunt:
      let bit = if scan.sim.exposedNow: 1 else: 0
      if scan.ribbonState < 0:
        replay.exposureRibbon.add(scan.sim.tickCount)
        scan.ribbonState = bit
        scan.ribbonRun = 1
      elif bit == scan.ribbonState:
        inc scan.ribbonRun
      else:
        replay.exposureRibbon.add(scan.ribbonRun)
        replay.exposureRibbon.add(scan.ribbonState)
        scan.ribbonState = bit
        scan.ribbonRun = 1
    ...
    for event in stepBeats:
      ...
      # `BeatKinds` is the closed set of scrubber markers, and a `spotted`
      # event only makes one when the tracker flagged it (the first spot of a
      # hider in a game, or the start of a long exposure run) — otherwise a
      # busy hunt would bury the timeline.
      let kind = event["k"].getStr()
      var isBeat = false
      for candidate in BeatKinds:
        if candidate == kind:
          isBeat = true
      if kind == "spotted" and not event{"beat"}.getBool():
        isBeat = false
      if isBeat:
        replay.beatEvents.add(event)
    for event in stepBeats:
      # A lull is 72 consecutive ticks with no grab, lock, vault, spotted,
      # lost or sealed event.
      let kind = event["k"].getStr()
      if kind in ["grab", "lock", "vault", "spotted", "lost", "sealed",
          "unsealed", "release", "gameover"]:
  ...
  if scan.ribbonState >= 0 and scan.ribbonRun > 0:
    replay.exposureRibbon.add(scan.ribbonRun)
    replay.exposureRibbon.add(scan.ribbonState)
    scan.ribbonRun = 0
