...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(HnsError, "OS entropy source unavailable")
...
proc echoStartupConfig(
  ...
  echo "hide-and-seek config: host=", runtimeConfig.host,
...
when isMainModule:
  let
    ...
    localReplayPath =
      if runtimeConfig.replayUri.len > 0:
        getTempDir() / ("hns-replay-" & $getCurrentProcessId() &
          ".bitreplay")
      else:
        ""
  ...
  if seedPinned(runtimeConfig.config):
    ...
    ## seed-derived — the room pick (`pool[seed mod 3]`) and its
    ## replay-pinned mapSpec — so the randomized seed must already be in
    ## place or every process would draw the same room.
  ...
  let loadReplayPath =
    if runtimeConfig.replayMode:
      let path = getTempDir() / ("hns-load-replay-" &
  ...
  echo "starting hide-and-seek on ", runtimeConfig.host, ":", runtimeConfig.port
  runServerLoop(
    runtimeConfig.host,
    runtimeConfig.port,
    config,
