import
  std/json,
  hns/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with hns_error_len() == 0 and no diagnostic at all). The
...
proc hnsLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "hns_load_replay", cdecl.} =
  try:
    lastError = ""
    ...
    # THE LOAD-TIME PRE-SCAN. `initReplayRuntime` starts the whole-match walk
    # and the hosted viewer normally advances it a slice per presentation
    # frame; here it is finished BEFORE the first frame is drawn. A
    # hide-and-seek episode is 2160 ticks x 6 cogs of integer work plus fov —
    # tens of milliseconds in wasm — and paying it up front is what lets the
    # EXPOSURE RIBBON, the momentum series and the scrubber beats draw at FULL
    # WIDTH on frame 0 instead of growing in as playback passes them.
    stampStage("pre-scan the episode")
    replay.advanceReplayScan(int.high)
    viewer = initGlobalViewerState()
    runtimeLoaded = true
    let mapNote =
      " (room " & game.roomName & " " & $game.gameMap.width & "x" &
        $game.gameMap.height & ")"
...
proc hnsInput(data: ptr uint8, length: cint)
    {.exportc: "hns_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc hnsFrame(): cint {.exportc: "hns_frame", cdecl.} =
...
proc hnsPacketPointer(): ptr uint8
    {.exportc: "hns_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc hnsPacketLength(): cint {.exportc: "hns_packet_len", cdecl.} =
  cint(packet.len)

proc hnsMismatchTick(): cint {.exportc: "hns_mismatch_tick", cdecl.} =
  if runtimeLoaded:
    cint(replay.hashMismatchTick)
  else:
    -1

proc hnsErrorPointer(): ptr uint8 {.exportc: "hns_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc hnsErrorLength(): cint {.exportc: "hns_error_len", cdecl.} =
  cint(lastError.len)

proc hnsStagePointer(): ptr uint8 {.exportc: "hns_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike hns_error_*, this stays
...
proc hnsStageLength(): cint {.exportc: "hns_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # module stays alive and JS keeps calling hns_load_replay/hns_frame. The
