...
function stageNote() {
  ...
  try {
    var length = Module._hns_stage_len ? Module._hns_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._hns_stage_ptr();
...
function runtimeError() {
  var length = Module._hns_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._hns_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._hns_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._hns_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._hns_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._hns_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._hns_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function applyInputNow() {
  ...
  try {
    if (Module._hns_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._hns_mismatch_tick()
...
importScripts('./wire_constants.js', './broadcast_core.js', './hns_replay.js');
