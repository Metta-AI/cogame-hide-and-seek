...
# use-after-free fixed in hns_replay.nim (emscripten_exit_with_live_runtime)
...
# hns_error_len(). Aborting keeps linear memory intact, and the page reads
# hns_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "hns_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_hns_load_replay,_hns_frame,_hns_input,_hns_packet_ptr,_hns_packet_len,_hns_mismatch_tick,_hns_error_ptr,_hns_error_len,_hns_stage_ptr,_hns_stage_len
  """).replace("\n", " ")
)
